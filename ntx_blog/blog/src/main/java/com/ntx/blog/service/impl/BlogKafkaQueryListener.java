...
public class BlogKafkaQueryListener {
    ...
    public void topicListener3(ConsumerRecord<String, String> record) throws IOException {
        ...
        //在redis中存入list，用于消息通知
        Integer blogId = comment.getBlogId();
        //查询评论的文章
        Query queryBlog = new Query();
        queryBlog.addCriteria(Criteria.where("_id").is(blogId));
        BlogDTO blogDTO = mongoTemplate.findOne(queryBlog, BlogDTO.class);
        Integer bloggerId = null;
        if (blogDTO != null) {
            bloggerId = blogDTO.getBloggerId();
        }
        //如果这条评论不是博主发的
        if(!Objects.equals(comment.getUserId(), bloggerId)){
            //处理业务
            String redisKey = BLOG_INFORM_KEY + bloggerId;
            stringRedisTemplate.opsForList().leftPush(redisKey, String.valueOf(comment.getId()));
//            //评论通知构成：评论人名称，评论人头像，评论时间，评论的博客，评论的内容
//            List<TUser> commentUserList = userClient.getByIds(Collections.singletonList(comment.getUserId()));
//            for (TUser user : commentUserList) {
...
//                if (blogDTO != null) {
//                    messageDTO.setCommentBlogTitle(blogDTO.getTitle());
//                }
